...
impl Simd for Sse4_2 {
    ...
    fn permute_within_blocks_f32x4(self, a: f32x4<Self>, b: mask32x4<Self>) -> f32x4<Self> {
        unsafe {
            let shuffle_mask = _mm_add_epi32(
                _mm_mullo_epi32(b.into(), _mm_set1_epi32(0x04040404)),
                _mm_set1_epi32(0x03020100),
            );
            _mm_castsi128_ps(_mm_shuffle_epi8(_mm_castps_si128(a.into()), shuffle_mask))
                .simd_into(self)
        }
    }
    #[inline(always)]
    ...
    fn permute_within_blocks_f64x2(self, a: f64x2<Self>, b: mask64x2<Self>) -> f64x2<Self> {
        unsafe {
            let shuffle_mask = _mm_add_epi64(
                _mm_slli_epi64::<3>(_mm_shuffle_epi8(
                    b.into(),
                    _mm_setr_epi8(0, 0, 0, 0, 0, 0, 0, 0, 8, 8, 8, 8, 8, 8, 8, 8),
                )),
                _mm_set1_epi64x(0x0706050403020100),
            );
            _mm_castsi128_pd(_mm_shuffle_epi8(_mm_castpd_si128(a.into()), shuffle_mask))
                .simd_into(self)
        }
    }
    #[inline(always)]
